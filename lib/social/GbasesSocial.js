...
class GbaseSocial {
    ...
    vkListPurchases(skip=0, limit=20, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 185));
        ...
        this._gbaseApi._networkManager.doSendRequest(`${URIS['vkJobs.listPurchases']}?offset=${skip}&limit=${limit}`, null, callbackFn);
    }
    ...
    vkConsumePurchase(purchaseNum, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 189));
        } else if(!this._gbaseApi.currentProfile){
        ...
        this._gbaseApi._networkManager.doSendRequest(`${URIS['vkJobs.consumePurchase']}?purchasenum=${purchaseNum}`, null, callbackFn);
    }
    ...
    okListPurchases(skip=0, limit=20, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 193));
        } else if(!this._gbaseApi.currentProfile){
        ...
        this._gbaseApi._networkManager.doSendRequest(`${URIS['okJobs.listPurchases']}?offset=${skip}&limit=${limit}`, null, callbackFn);
    }
    ...
    okConsumePurchase(purchaseNum, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 197));
        } else if(!this._gbaseApi.currentProfile){
        ...
        this._gbaseApi._networkManager.doSendRequest(`${URIS['okJobs.consumePurchase']}?purchasenum=${purchaseNum}`, null, callbackFn);
    }
}

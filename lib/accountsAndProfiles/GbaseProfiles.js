...
class GbaseProfiles {
    ...
    create(callback=NOOP){
        ...
        let callbackFn = (err, body) => {
            if(err){
                ...
                if(body.disallowDirectProfileExposure){
                    this.disallowDirectProfileExposure = true;
                }
                callback(null, new GbaseResponse(true, { originalResponse: body }));
            }
        };

        this._gbaseApi._networkManager.doSendRequest(URIS['profile.createProfile'], null, callbackFn);
    }
    ...
    getp(callback=NOOP){
        ...
        this._gbaseApi._networkManager.doSendRequest(URIS['profile.getProfile'], null, callbackFn);
    }
    ...
    setp(profileData, publicProfileData, rating, mmr, wlRate, ver, callback=NOOP){
        ...
        } else if(this.disallowDirectProfileExposure){
            return callback(new GbaseError('Profile direct modification is not allowed (by configuration). It\'s only can be done with cloud functions', 52));
        } else if(!this._gbaseApi.currentProfile){
        ...
        if(Object.keys(toSet).length){
            this._gbaseApi._networkManager.doSendRequest(URIS['profile.setProfile'], toSet, callbackFn);
        } else {
    ...
    update(profileData, publicProfileData, rating, mmr, wlRate, ver, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Profile direct modification is not allowed (by configuration). It\'s only can be done with cloud functions', 56));
        } else if(!this._gbaseApi.currentProfile){
        ...
        if(Object.keys(toUpdate).length){
            this._gbaseApi._networkManager.doSendRequest(URIS['profile.updateProfile'], toUpdate, callbackFn);
        } else {
    ...
    getPublic(targetHumanId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Records direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 131));
        ...
        this._gbaseApi._networkManager.doSendRequest(`${URIS['profile.getPublicProfile']}?hid=${targetHumanId}`, null, callbackFn);
    }
}

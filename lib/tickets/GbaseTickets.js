...
class GbaseTickets {
    ...
    sendTicket(receiverHumanId, header, payload, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 201));
    ...
    sendTicketVk(receiverVkId, header, payload, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 205));
        } else if(!this._gbaseApi.currentProfile){
    ...
    sendTicketOk(receiverOkId, header, payload, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 209));
        } else if(!this._gbaseApi.currentProfile){
    ...
    sendTicketFb(receiverFbId, header, payload, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 213));
        } else if(!this._gbaseApi.currentProfile){
    ...
    listSendedTickets(skip=0, limit=20, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 217));
        } else if(!this._gbaseApi.currentProfile){
    ...
    listReceivedTickets(skip=0, limit=20, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 221));
        } else if(!this._gbaseApi.currentProfile){
    ...
    confirmTicket(ticketId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 225));
        } else if(!this._gbaseApi.currentProfile){
    ...
    confirmTicketVk(ticketId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 229));
        } else if(!this._gbaseApi.currentProfile){
    ...
    confirmTicketOk(ticketId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 233));
        } else if(!this._gbaseApi.currentProfile){
    ...
    confirmTicketFb(ticketId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 237));
        } else if(!this._gbaseApi.currentProfile){
    ...
    rejectTicket(ticketId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 241));
        } else if(!this._gbaseApi.currentProfile){
    ...
    rejectTicketVk(ticketId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 245));
        } else if(!this._gbaseApi.currentProfile){
    ...
    rejectTicketOk(ticketId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 249));
        } else if(!this._gbaseApi.currentProfile){
    ...
    rejectTicketFb(ticketId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 253));
        } else if(!this._gbaseApi.currentProfile){
    ...
    dischargeTicket(ticketId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 257));
        } else if(!this._gbaseApi.currentProfile){
    ...
    dismissTicket(ticketId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 261));
        } else if(!this._gbaseApi.currentProfile){
    ...
    releaseTicket(ticketId, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 265));
        } else if(!this._gbaseApi.currentProfile){

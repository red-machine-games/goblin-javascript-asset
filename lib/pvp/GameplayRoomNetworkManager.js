...
const RECONNECT_INTERVAL_MS = 1000,
    RECONNECT_MAX_MS = 20 * 1000;

const MAX_PAYLOAD = 6144,
...
class GameplayRoomNetworkManager extends EventEmitter{
        ...
        if(this._wsConnection && this._wsConnection.readyState === 1){
            ...
            if(this._wsPingSent - this._heartbeatLastMsg >= PING_INTERVAL_MS * 5){
                ...
                this._wsConnection.send(
                    JSON.stringify(this._wsActivityPaused ? { pm: 1 } : { ping: this._wsPing }),
                    WS_SEND_OPTS
                );
            }
        }
    ...
    _makeWsConnection(pushPrevLock){
        ...
        function handleError(err){
            ...
            laThis.wsSequence = 0;
            if(laThis._wsConnection){
                if(laThis._wsConnection.readyState < 2){
                    laThis._wsConnection.terminate();
                }
                laThis._wsConnection = null;
                if(laThis._wsReconnectingFrom === 0){
                    laThis._wsReconnectingFrom = Date.now();
                    laThis.emit('disconnected');
                }
                if(Date.now() - laThis._wsReconnectingFrom + 1000 >= RECONNECT_MAX_MS){
                    laThis._wsReconnectingFrom = 0;
                    laThis.emit('error', err);
                    laThis.laClose(new GbaseError('Pair died due to connection problems', 314));
                } else {
                    setTimeout(() => laThis.reconnectWs(), RECONNECT_INTERVAL_MS);
                }
            } else {
                laThis.emit('error', err);
            }
        ...
        function handleMessage(msg){
            ...
            if(msg.startsWith('{') || msg.startsWith('[')){
                ...
                if(typeof msg.yrAvg === 'number' && typeof msg.oppAvg === 'number'){
                    laThis.myAveragePing = msg.yrAvg;
                    laThis.opponentAveragePing = msg.oppAvg;
        ...
        function handleClose(code, msg){
            ...
            function normalClose(){
                ...
                if(msg.gameIsOver){
                    laThis.laClose(msg.finalm, msg, true);
                } else {
                    laThis.laClose(msg.m, msg, true);
                }
            }
    ...
    sendWsMessage(theMessage, direct, toPush=false){
        ...
        if(this._wsConnection && this._wsConnection.readyState === 1 && this._wsConnectionIsReady && !this._wsActivityPaused){
            if(direct){
                let messageToSendStr = `-${JSON.stringify(typeof theMessage === 'object' ? theMessage : { message: theMessage })}`;
                if(Buffer.from(messageToSendStr).length > MAX_PAYLOAD){
                    throw new GbaseError(`Trying to send message size above MAX_PAYLOAD(${MAX_PAYLOAD})`, 313);
                }
                this._wsConnection.send(messageToSendStr);
            } else {
                ...
                let messageToSendStr = JSON.stringify(messageToSend);
                if(Buffer.from(messageToSendStr).length > MAX_PAYLOAD){
                    throw new GbaseError(`Trying to send message size above MAX_PAYLOAD(${MAX_PAYLOAD})`, 312);
                }
                this._wsConnection.send(messageToSendStr);
            }
            return true;
    ...
    _doSendRequest(uri, body, callback, _noHmacCheckEtc){
        ...
        theReq.set('X-Platform-Version', this._platformVersion);
        if(!_noHmacCheckEtc){
            theReq
    ...
    laClose(theMessage, originalResponse, doNotDispatchDisconnection){
        this.laClosed = true;
        clearInterval(this._heartbeatInterval);
        this._heartbeatIntervalDone = true;
        if(this._wsConnection){
            this._actuallyClose(doNotDispatchDisconnection);
        }
        if(theMessage instanceof GbaseError){
            this.emit('close', theMessage);
        } else {
            this.emit('close', new GbaseResponse(true, originalResponse ? { endMessage: theMessage, originalResponse } : { endMessage: theMessage }));
        }
    }
    _actuallyClose(doNotDispatchDisconnection){

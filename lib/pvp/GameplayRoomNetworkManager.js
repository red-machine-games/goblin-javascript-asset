...
class GameplayRoomNetworkManager extends EventEmitter{
    ...
    _makeWsConnection(pushPrevLock){
        ...
        function handleClose(code, msg){
            ...
            function normalClose(){
                ...
                if(msg.gameIsOver){
                    laThis.laClose(msg.finalm);
                } else {
                    laThis.laClose(msg.m);
                }
            }
    ...
    _pokeWsQueue(){
        if(this._wsQueue.length){
            ...
            if(this.sendWsMessage(theMessage.theMessage, theMessage.isDirect, true)){
                this._pokeWsQueue();
            }
        }
    }
    sendWsMessage(theMessage, direct, toPush=false){
        ...
        if(this._wsConnection && this._wsConnection.readyState === 1 && this._wsConnectionIsReady && !this._wsActivityPaused){
            if(direct){
                this._wsConnection.send(`-${JSON.stringify(typeof theMessage === 'object' ? theMessage : { m: theMessage })}`);
            } else {
                this.wsSequence++;
                let messageToSend = { mysq: this.wsSequence, m: (typeof theMessage === 'object') ? theMessage : { m: theMessage } },
    ...
    backupWsMessages(fromSequence, opponentSequence, onlyForward=false){
        ...
        if(fromSequence === this.wsSequence || (onlyForward && this.wsSequence >= fromSequence)){
            ...
            let okay = false;
            for(let i = 0 ; i < this._wsRememberingQueue.length ; i++){
                if(this._wsRememberingQueue[i].sequence === fromSequence){
                    this._wsQueue = this._wsRememberingQueue.slice(i, this._wsRememberingQueue.length - 1).concat(this._wsQueue);
                    this._pokeWsQueue();
                    okay = true;
                }
            }

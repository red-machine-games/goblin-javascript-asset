...
class GbasePvp extends EventEmitter{
    ...
    constructor(gbaseApiInstance, gbasePvpApiInstance, pvpRoomAddress, pvpRoomKey, _deferredConnection){
        ...
        this._networkManager.on('disconnected', () => {
            if(this._currentlyOnPause === false){
                this._currentlyOnPause = true;
                this.emit('paused', 'Game paused due to disconnection');
            }
        });
    ...
    doConnect(payloadObject){
        ...
        function checkIamThereAlready(){
            let callbackFn = err => {
                if(err){
                    if(err instanceof GbaseError && err.details && err.details.status === 400 && ([616, 422].includes(err.details.body.index))){
                        hangListeners();
                        releaseBooking();
                    } else {
        ...
        function _onMessage(theMessage, isDirect){
            if(isDirect){
                ...
                if(theMessage.p === 4 && theMessage.c === 3 && theMessage.paused != null){
                    return handlePause(!!theMessage.paused, theMessage.turn, theMessage.m, theMessage.to, theMessage.from);
                }
                ...
                if(theMessage.c === -1){
                    return leThis._networkManager.laClose({ message: theMessage.m, autoCloseMessage: theMessage.dsp }, theMessage);
                }
                if(theMessage.c >= 0 && theMessage.c <= 4){
                    let _dispatchUnpaused = false;

                    if(theMessage.paused != null){
                        theMessage.paused = !!theMessage.paused;
                        _dispatchUnpaused = (leThis._currentlyOnPause !== theMessage.paused);
                        leThis._currentlyOnPause = theMessage.paused;
                        leThis._networkManager.makePaused(theMessage.paused);
                    }
                    if(theMessage.state != null){
                        ...
                        if(theMessage.state.opponentPayload != null){
                            leThis._opponentPayload = theMessage.state.opponentPayload;
                        }
                        if(!leThis._begin){
                            leThis._begin = true;
                            leThis.emit('begin');
                        ...
                        if(theMessage.paused != null && theMessage.to != null && !theMessage.paused && _dispatchUnpaused){
                            leThis.emit('unpaused', null, theMessage.from, theMessage.to);
                        }
                    }
                    if(theMessage.m){
                ...
                } else if(theMessage.m != null){
                    leThis.emit('turn-message', theMessage.m);
                }
            }
        }
        function handlePause(isPaused, myTurn, messageThatWillGoOut, toTimestamp, fromTimestmap){
            var _mkPaused = leThis._networkManager.makePaused(isPaused);
            if(isPaused){
                if(!leThis._currentlyOnPause){
                    leThis._currentlyOnPause = !!isPaused;
                    if(_mkPaused){
                        leThis.emit('paused', messageThatWillGoOut, fromTimestmap);
                    }
                }
            } else {
                if(leThis._currentlyOnPause){
                    leThis._currentlyOnPause = !!isPaused;
                    let _buOkay = leThis._networkManager.backupWsMessages(myTurn);
                    leThis.emit('unpaused', messageThatWillGoOut, fromTimestmap, toTimestamp);
                    if(!_buOkay){
                        leThis.emit('sync');
                    }
                }
        ...
        function _onConnectionClose(theResponse){
            ...
            if(theResponse && theResponse.details.endMessage !== 'Forced to close client-side'){
                leThis.emit('finish', theResponse);
            }
        }
        function _onError(theError){
            if(theError instanceof Error) {
                leThis.emit('error', { message: 'Connection was interrupted', originalCode: theError.code });
            } else {
     ...
     */
    get isPaused(){
        return this._currentlyOnPause || false;
    }

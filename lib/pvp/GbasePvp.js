...
class GbasePvp extends EventEmitter{
    ...
    constructor(gbaseApiInstance, gbasePvpApiInstance, pvpRoomAddress, pvpRoomKey, _deferredConnection){
        ...
        this._networkManager.on('disconnected', () => this._currentlyOnPause = true);
    ...
    doConnect(payloadObject){
        ...
        function _onMessage(theMessage, isDirect){
            if(isDirect){
                ...
                if(theMessage.p === 4 && theMessage.c === 3 && theMessage.paused != null){
                    return handlePause(!!theMessage.paused, theMessage.turn, theMessage.m, theMessage.at, theMessage.from);
                }
                ...
                if(theMessage.c >= 0 && theMessage.c <= 4){
                    ...
                    if(theMessage.state != null){
                        ...
                        if(theMessage.paused != null && theMessage.at != null && !theMessage.paused && _dispatchUnpaused){
                            leThis.emit('unpaused', null, theMessage.at, theMessage.from);
                        }
                    }
                    if(theMessage.m){
        ...
        function handlePause(isPaused, myTurn, messageThatWillGoOut, atTimestamp, fromTimestmap){
            if(isPaused){
                if(!leThis._currentlyOnPause){
                    leThis._currentlyOnPause = isPaused;
                    if(leThis._networkManager.makePaused(true)){
                        leThis.emit('paused', messageThatWillGoOut, atTimestamp);
                    }
                }
            } else {
                if(leThis._currentlyOnPause){
                    ...
                    leThis.emit('unpaused', messageThatWillGoOut, atTimestamp, fromTimestmap);
                    if(!_buOkay){
                        leThis.emit('sync');
                    }
                }

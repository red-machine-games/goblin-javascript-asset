...
class GbasePvpApi {
    ...
    checkBattleNoSearch(callback=NOOP){
        return someUtils.promisify(callback, callback => {
            if(!this._gbaseApi.currentUnicorn){
                ...
                return callback(new GbaseError('Direct access is not allowed (by configuration). It only can be done done with cloud functions', 141));
            ...
            this._gbaseApi._networkManager.doTheRequest(`${URIS['pvp.checkBattleNoSearch']}`, null, callbackFn);
        });
    }
    ...
    dropMatchmaking(callback=NOOP){
        return someUtils.promisify(callback, callback => {
            if(!this._gbaseApi.currentUnicorn){
                ...
                return callback(new GbaseError('Direct access is not allowed (by configuration). It only can be done done with cloud functions', 144));
            } else if(!this._gbaseApi.currentProfile){
            ...
            this._gbaseApi._networkManager.doTheRequest(`${URIS['pvp.dropMatchmaking']}`, null, callbackFn);
        });
    }
     ...
     * @returns {Promise} - returns promise if no callback provided
     */
    withOpponent(fromSegment=DEFAULT_RECORDS_SEGMENT, strategy, rangesOrDetails, timeLimitSec=60, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            if(this._currentlySearchingPvp){
                ...
                return callback(new GbaseError('Direct access is not allowed (by configuration). It only can be done done with cloud functions', 150));
            } else if(!this._gbaseApi.currentProfile){
                ...
                if(fromSegment && typeof fromSegment !== 'string'){
                    return callback(new GbaseError('Argument fromSegment is invalid', 307));
                }
                if(!strategy || typeof strategy !== 'string'){
                    return callback(new GbaseError('Argument strategy is invalid', 308));
                }
            }
            ...
            function doSearch(){
                leThis._gbaseApi._networkManager.doTheRequest(`${URIS['pvp.searchForOpponent']}?segment=${fromSegment}&strat=${strategy}`, rangesOrDetails, _aUniversalCallback);
            }
            function doStopSearch(){
                clearTimeout(_to);

                leThis._gbaseApi._networkManager.doTheRequest(`${URIS['pvp.stopSearchingForOpponent']}`, null, _aUniversalCallback);
            }
            function doAcceptGame(){
                clearTimeout(_to);

                leThis._gbaseApi._networkManager.doTheRequest(`${URIS['pvp.acceptMatch']}`, null, _aUniversalCallback);
            }
            function doWaitForOpponent(){
                clearTimeout(_to);

                leThis._gbaseApi._networkManager.doTheRequest(`${URIS['pvp.waitForOpponentToAccept']}`, null, _aUniversalCallback);
            }
     ...
     * @returns {Promise} - returns promise if no callback provided
     */
    withBotOpponent(rangesOrDetails, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            if(this._currentlySearchingPvp){
                ...
                return callback(new GbaseError('Direct access is not allowed (by configuration). It only can be done done with cloud functions', 158));
            } else if(!this._gbaseApi.currentProfile){
                return callback(new GbaseError('You need to get profile first', 159));
            } else if(!this._gbaseApi.disallowDirectPvpMatchmakingExposure){
                if(!rangesOrDetails || !(rangesOrDetails instanceof GbaseRangePicker) || !rangesOrDetails.rgs.length){
            ...
            _pvpVersusSelfCommon(this, (leThis, callbackFn) => leThis._gbaseApi._networkManager.doTheRequest(
                `${URIS['pvp.searchForBotOpponent']}?strat=${MATCHMAKING_STRATEGIES_MAP.BY_RATING}`, rangesOrDetails,
                callbackFn
            ), callback);
        });
    }
    ...
    stopSearchingForOpponent(){
        if(!this._currentlySearchingPvp){
            ...
            return callback(new GbaseError('Direct access is not allowed (by configuration). It only can be done done with cloud functions', 164));
        } else if(!this._gbaseApi.currentProfile){
    ...
    beginVersusSelf(targetHumanId, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            if(this._currentlySearchingPvp){
                ...
                return callback(new GbaseError('Direct access is not allowed (by configuration). It only can be done done with cloud functions', 169));
            } else if(!this._gbaseApi.currentProfile){
    ...
    beginOnAddressAndKey(pvpRoomData, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            if(this._currentlySearchingPvp){
                ...
                return callback(new GbaseError('Direct access is not allowed (by configuration). It only can be done done with cloud functions', 175));
            } else if(!this._gbaseApi.currentProfile){
    ...
    battlesList(skip=0, limit=20, onlyAuto, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            if(!this._gbaseApi.currentUnicorn){
                ...
                return callback(new GbaseError('Direct access is not allowed (by configuration). It only can be done done with cloud functions', 179));
            } else if(!this._gbaseApi.currentProfile){
...
function _pvpVersusSelfCommon(leThis, requestLambda, callback){
    var theBody;
    ...
    function doAccept(){
        ...
        leThis._gbaseApi._networkManager.doTheRequest(`${URIS['pvp.acceptMatch']}`, null, callbackFn);
    }

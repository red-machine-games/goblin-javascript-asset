...
class GbasePvpApi {
    ...
    withOpponent(fromSegment=DEFAULT_RECORDS_SEGMENT, strategy, rangesOrDetails, timeLimitSec=60, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            ...
            function doSearch(){
                leThis._gbaseApi._networkManager.doSendRequest(`${URIS['pvp.searchForOpponent']}?segment=${fromSegment}&strat=${strategy}`, rangesOrDetails, _aUniversalCallback);
            }
    ...
    withBotOpponent(rangesOrDetails, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            ...
            _pvpVersusSelfCommon(this, (leThis, callbackFn) => leThis._gbaseApi._networkManager.doSendRequest(

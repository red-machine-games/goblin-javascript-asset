...
class GbaseMatchmaking {
    ...
    matchPlayer(fromSegment=DEFAULT_RECORDS_SEGMENT, strategy, rangesOrDetails, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            ...
            this._gbaseApi._networkManager.doSendRequest(`${URIS['mm.matchPlayer']}?segment=${fromSegment}&strat=${strategy}`, rangesOrDetails, callbackFn);
        });
    }
    ...
    matchBot(rangesOrDetails, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            ...
            this._gbaseApi._networkManager.doSendRequest(`${URIS['mm.matchBot']}?strat=${MATCHMAKING_STRATEGIES_MAP.BY_RATING}`, rangesOrDetails, callbackFn);
        });
    }
}

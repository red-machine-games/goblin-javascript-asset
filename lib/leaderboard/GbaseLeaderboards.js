...
class GbaseLeaderboards {
    ...
    postRecord(value, intoSegment=DEFAULT_RECORDS_SEGMENT, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Records direct publish is not allowed (by configuration). It\'s only can be done with cloud functions', 65));
        ...
        this._gbaseApi._networkManager.doSendRequest(`${URIS['tops.postARecord']}?value=${value}&segment=${intoSegment}`, {}, callbackFn);
    }
    ...
    getSelfRecord(fromSegment=DEFAULT_RECORDS_SEGMENT, callback=NOOP){
        ...
        this._gbaseApi._networkManager.doSendRequest(`${URIS['tops.getPlayerRecord']}?segment=${fromSegment}`, null, callbackFn);
    }
    ...
    getLeaderboardOverall(fromSegment=DEFAULT_RECORDS_SEGMENT, skip=0, limit=20, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Records direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 99));
        } else if(!this._gbaseApi.currentProfile){
    ...
    removeSelfRecord(fromSegment=DEFAULT_RECORDS_SEGMENT, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Records direct publish is not allowed (by configuration). It\'s only can be done with cloud functions', 75));
        } else if(!this._gbaseApi.currentProfile){
        ...
        this._gbaseApi._networkManager.doSendRequest(`${URIS['tops.removeRecord']}?segment=${fromSegment}`, null, callbackFn);
    }
    ...
    getSomeonesRating(targetHumanId, fromSegment=DEFAULT_RECORDS_SEGMENT, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Records direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 115));
        } else if(!this._gbaseApi.currentProfile){
    ...
    getLeadersWithinFriends(fromSegment=DEFAULT_RECORDS_SEGMENT, skip=0, limit=20, callback=NOOP){
        if(!this._gbaseApi.currentUnicorn){
            ...
            return callback(new GbaseError('Records direct access is not allowed (by configuration). It\'s only can be done with cloud functions', 117));
        } else if(!this._gbaseApi.currentProfile){

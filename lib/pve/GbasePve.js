...
class GbasePve {
    ...
    begin(beginParams, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            ...
            this._gbaseApi._networkManager.doTheRequest(`${URIS['pve.beginSimple']}`, beginParams, callbackFn);
        });
    }
    ...
    act(actParams, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            ...
            this._gbaseApi._networkManager.doTheRequest(`${URIS['pve.actSimple']}`, actParams, callbackFn);
        });
    }
    ...
    listBattles(skip=0, limit=20, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            if(!this._gbaseApi.currentUnicorn){
                ...
                return callback(new GbaseError('Direct access is not allowed (by configuration). It only can be done done with cloud functions', 276));
            } else if(!this._gbaseApi.currentProfile){
            ...
            this._gbaseApi._networkManager.doTheRequest(`${URIS['pve.listBattles']}`, null, callbackFn);
        });
    }
}

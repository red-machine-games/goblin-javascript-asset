...
class GbaseTickets {
    ...
    enterChatGroup(header){
        if(!this._gbaseApi.currentUnicorn){
            throw new GbaseError('You need to auth first', 9999);
        } else if(!this._gbaseApi.currentProfile){
            throw new GbaseError('You need to get profile before', 9999);
        } else if(!header){
            throw new GbaseError('Argument header is invalid', 9999);
        }

...
class GroupChat extends EventEmitter{
    ...
    loadHistory(callback=NOOP){
        return someUtils.promisify(callback, callback => {
            if(!this._gbaseApi.currentUnicorn){
                return callback(new GbaseError('You need to auth first', 319));
            } else if(!this._gbaseApi.currentProfile){
                return callback(new GbaseError('You need to get profile before', 320));
            } else if(!this._subscribed){
                return callback(new GbaseError('You have to listen first', 321));
            }
    ...
    post(message, callback=NOOP){
        return someUtils.promisify(callback, callback => {
            if(!this._gbaseApi.currentUnicorn){
                return callback(new GbaseError('You need to auth first', 322));
            } else if(!this._gbaseApi.currentProfile){
                return callback(new GbaseError('You need to get profile before', 323));
            } else if(!message){
                return callback(new GbaseError('Argument message is invalid', 324));
            }

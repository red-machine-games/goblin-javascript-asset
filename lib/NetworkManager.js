...
class NetworkManager {
    ...
    heartbeat(){
        var now = Date.now(),
            delta = now - this.lastAct;

        if(this.currentUnicorn && (delta >= PING_DELTA_TIME_MS || delta < NO_INPUT_DELTA_TIME_MS)){
            this.doPingServer();
        }
    }
    doPingServer(){
        this.doTheRequest(this._pingServerUri, null, NOOP);
    }
        ...
        if(theError.details.body && theError.details.body.index){
            switch(theError.details.body.index){
                ...
                default:
                    this.theQueueCurrentlyProcessing = false;
                    this.dropQueue();
                    callback(new GbaseError('Gbase backend respond with error', 11, { originalError: theError.details.body }));
            }
        } else {
            this.dropQueue();
            callback(new GbaseError('Unknown error', 5));
        }
    }

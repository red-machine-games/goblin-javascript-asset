...
class NetworkManager extends EventEmitter{
    ...
    heartbeat(){
        ...
        if(this.currentUnicorn && (deltaPing >= PING_DELTA_TIME_MS && deltaAct < NO_INPUT_DELTA_TIME_MS)){
            this.lastPing = now;
            this.doPingServer();
        }
    }
    ...
    doSendRequest(uri, body, callback, theTry=0){
        ...
        let callbackFn = (err, res) => {
            if(pingRequest){
                return callback((err && err.status === 401) ? new GbaseError('Ping unauthorized', 311) : null, null);
            }
            if(err && !err.status){
                if(err.timeout){

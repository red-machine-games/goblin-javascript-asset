...
class NetworkManager extends EventEmitter{
    ...
    sendRequestOutOfQueue(uri, body, callback){
        let callbackFn = (err, res) => {
            if(err && !err.status){
                if(err.timeout){
                    callback(new GbaseError('Request timeout', 325));
                } else {
                    callback(new GbaseError('Some connection error occur', 326, err));
                }
            } else if(res.status === 200){
                callback(null, res.body);
            } else if(res.status === 401){
                callback(new GbaseError('Auth error occur', 327, { status: res.status, body: res.body }));
            } else {
                callback(new GbaseError('Some logic error occur', 328, { status: res.status, body: res.body }));
            }
        };
